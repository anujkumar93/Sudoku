public class Sudoku_MRVBackTrackingWithAC3 {
	int guess = 0;

	public int mrvBacktrackingSearchWithAC3(String[] args) {
		SudokuIO.makeSudokuPuzzle(args);
		alterVarDomain();
		recursiveBckTrackingWithMrvAC3(SudokuIO.vars, SudokuIO.varDomain);
		return guess;
	}
	...
	public int[] recursiveBckTrackingWithMrvAC3(int[] vars, boolean[][] varDomain) {
			...
			for (int j = 1; j < 10; j++) {
				if (varDomain[mostConstrainedCell][j - 1]){
					...
					if (checkConstraints(mostConstrainedCell, vars, j)) {
						correct = true;
						tempDomain = executeAC3(mostConstrainedCell, j, varDomain);
						result = recursiveBckTrackingWithMrvAC3(vars, tempDomain);
						if (result == null) {
	...
	private boolean[][] executeAC3(int firstUnassigned, int number, boolean[][] domain) {
	...
	public static void main(String[] args) {
		Sudoku_MRVBackTrackingWithAC3 sbt = new Sudoku_MRVBackTrackingWithAC3();
		sbt.mrvBacktrackingSearchWithAC3(args);
	}
}
